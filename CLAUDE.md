...
## Current status
See `ONBOARDING.md` §0 and the newest note in vault `Memory/`. **Merged product (DEC-003, accepted 26.09): "Dresden mit Kind"** — family hub (courses, events, bilingual communities, library + Health & services pillar) with the **"Ask for me"** feature (HalloTermin engine: an honest German AI assistant phones the place and returns the answer in the parent's language). UI languages by Sunday: en, de, ru, uk, ar (RTL), tr. **The UI source is vault `Concepts/Frontend and UX Plan v3 (merged).md`** (routes, data contract, Lovable Knowledge + prompts); Plan v2 is only the design base. Live: Supabase `weekender-build` (ref ycyrtlzympxzlfcocazh, merge migration `20260926200000_merged_family_hub.sql` applied; data per pillar in vault `Concepts/Family Hub Data.md`), n8n workflows 01/04/05/06 (10 task types incl. `course_enquiry` + `kita_enquiry`, intake prefill from a provider page), voice relay in `services/voice-relay/` (local; public hosting per `docs/deploy-relay.md` still open), Lovable app **live at https://dresden-mit-kind.lovable.app** (TanStack Start with SSR; code in the private repo AbdulrahmanBUW/hallotermin-paper, synced both ways; UI strings are pushed from `docs/i18n/ui-strings.json`, never rewritten in Lovable; team rules for Lovable in that repo's `AGENTS.md`). Prompts actually sent are in Plan v3 section G; P12 polish is next (Anastasia). Full loop tested on a real listing in vault `Runs/RUN-023 Merged flow E2E.md`. Test calls on real listings set "checked by phone" (DEF-047) → reset `resources.last_checked_at` / `last_check_outcome` after tests.
