...
sources: [anti-ai-slop-ui-ux, component-reference-design, ui-ux-pro-max, "[[Frontend and UX Plan v2]]", "[[Accessibility and RTL Review]]", "[[Merged Concept]]", "[[DEC-003 Merged concept]]", "[[Idea B - Dies-Das-Ana-Nas]]", "[[Pitch Kit v2 (merged)]]", "[[Task Types - How to extend]]", supabase/migrations/20260926200000_merged_family_hub.sql, supabase/migrations/20260926210000_credits_translate_practice_only.sql, docs/i18n/README.md, docs/deploy-relay.md, docs/i18n/ui-strings.json, n8n/workflows/04-task-intake.json, services/voice-relay/README.md, services/voice-relay/task-templates.js, services/voice-relay/public/index.html, "[[RUN-022 UI plan v3 merged]]", "[[RUN-025 Data round 2 and review sheets]]", supabase/migrations/20260926220000_checked_badge_ignore_tests.sql, "[[Deploy, Domain and SEO]]"]
---
# Frontend and UX Plan v3 (merged): Dresden mit Kind + "Ask for me"

> **Build status (26 Sep, 18:50): built and live at https://dresden-mit-kind.lovable.app.** Lovable project "HalloTermin Paper" (TanStack Start with SSR), GitHub sync to the private repo `AbdulrahmanBUW/hallotermin-paper`. Knowledge v3.1 is saved in Lovable. Sent and built, in this order: P1b, P2, P3, P7, P8, P9, P10, P11, P14, P4, P5, P6, **P15 dictation** and **P16 language switch without cookies** (both below P14), each adapted to SSR as written in this section. **Not sent yet:** P12 (polish, accessibility and RTL audit) and P13 (optional). Static files (robots.txt, sitemap.xml, og.png, favicon.svg, legal Markdown with placeholders) and all UI strings come from GitHub, not from Lovable. End-to-end test on the live site: [[RUN-026 Live site E2E call]]. Next owner of UI improvements: Anastasia (see the newest note in `Memory/`).

**v3.1 changes (26 Sep evening)** (review of sections A–F and H against the live database, the n8n exports, the relay and the Lovable repo; section G is updated separately):
- **SSR / TanStack Start rules:** the Lovable project renders on the server: file routes, no `window` / `localStorage` / `navigator` during render, `head()` instead of `document.title`, UI language from a cookie (B, C "Common", E row 2, F.1).
- **Service-role ban:** never import `client.server.ts` or `auth-middleware.ts`; no server functions that touch Supabase (F.1).
- **Email field removed:** no workflow sends email, and demo RLS lets anyone read `call_requests` (F.1, F.6).
- **Demo-only "Checked by phone" badge** plus an honest demo notice: every call this weekend is a browser role-play (A, C3, F.7).
- **Legal pages:** static `/impressum` (placeholders the team fills in) and `/datenschutz` (B).
- **Self-hosted fonts** via `@fontsource` packages, no Google Fonts link (D3).
- **SEO / `head()`:** titles, meta, `noindex` on `/r/*`, robots, sitemap (B; details in [[Deploy, Domain and SEO]]).
- **Live-state updates** ([[RUN-025 Data round 2 and review sheets]]): 162 resources / 92 family rows, 44 events to 29 Nov, demo listing "Olgas Musikstudio" exists, DEF-050 fixed in n8n 04, DEF-047 trigger fix (migration `20260926220000`), batch subtitles for assistant lines (n8n 06), `/demo` request reset, `bring_items_user` shape, brief-failure path (C7, C8, F, H).
- **Relay call page accessibility (must-fix 7) and Deepgram MIP opt-out: done** (E, H).

**What this is:** the design and build plan for the Lovable app after the merge ([[DEC-003 Merged concept]]). It **extends** [[Frontend and UX Plan v2]]: the design system "Bilingual paper", the intake, the approval card, the live call and the result screens stay. v3 adds the family hub around them (courses, events, communities, services, library, provider pages, suggestions) and the "Ask for me" entry from every provider and guide. It is a plan: nothing in Lovable is built yet except the P1 shell (a **TanStack Start** project with server rendering; GitHub sync connected; the 6 strings files pushed).
...
**Related:** [[Merged Concept]] · [[Frontend and UX Plan v2]] · [[Accessibility and RTL Review]] · [[Idea B - Dies-Das-Ana-Nas]] · [[Pitch Kit v2 (merged)]] · [[Task Types - How to extend]] · [[Data Model]] · [[Lovable - Practical Guide]] · [[AI Disclosure]] · [[Data minimisation - no symptoms]] · [[Never store call audio]] · [[Only book inside pre-approved windows]] · [[Read-back before booking]] · Runs: [[RUN-022 UI plan v3 merged]] · [[RUN-025 Data round 2 and review sheets]] · Deploy: [[Deploy, Domain and SEO]] · Defects: [[DEF-045 Plan v2 intake prompts do not match workflow 04]] · [[DEF-046 Non-Latin names reach the German greeting]] · [[DEF-047 Role-play calls mark real providers as checked by phone]] · [[DEF-050 Intake asks for the child's first name]]
...
| Freshness | — | "Checked by phone on …" badge from `resources.last_checked_at` (outcome only; v3.1: shown on the demo listing only while every call is a browser role-play) |
...
| `/find`, `/new` | Redirects to `/services` and `/ask` (old links keep working): `beforeLoad: () => { throw redirect({ to: "/services" }) }` as in `src/routes/demo.tsx`; `src/routes/new.tsx` is still the old intake page and becomes this redirect | — |
| `/impressum` | Impressum (§ 5 DDG), static page linked from the footer: responsible person, postal address and email as clearly marked placeholders "[Name]", "[Address]", "[Email]" that the team fills in; never invent personal data | — |
| `/datenschutz` | Privacy notice (Art. 13 GDPR), static page linked from the footer: what is stored (`call_requests` incl. the parent's name, readable by anyone in demo mode; no audio), processors (Supabase, n8n Cloud, Anthropic, Deepgram, Lovable hosting), no tracking, no analytics, only a functional language cookie | — |
| `*` | Friendly 404 | — |

**TanStack Start file routes (v3.1):** `/p/:id` = `src/routes/p.$id.tsx`, `/guides/:slug` = `src/routes/guides.$slug.tsx`, `/r/:id` = `src/routes/r.$id.tsx` (exists), `/impressum` = `src/routes/impressum.tsx`, `*` = `notFoundComponent` in `src/routes/__root.tsx` (exists). Query params are validated with `validateSearch` (zod) and read with `Route.useSearch()`, not `useSearchParams`. The home box text travels as history state: `navigate({ to: "/ask", state: { askText } })` with `askText` declared on `HistoryState`; it is lost on reload by design.

**Head and SEO (v3.1):** each route's `head()` sets the title, meta description and og tags ("… · Dresden mit Kind", never "HalloTermin" as in `r.$id.tsx` and the root `author` meta today). `noindex` on `/r/*`, `/ask` and `/suggest` (`/r/:id` shows request data). A static `robots.txt` (`Disallow: /r/`) and `sitemap.xml` (home, pillars, `/library`, `/guides/*`, `/p/*`). No hreflang while the language is not in the URL. Details: [[Deploy, Domain and SEO]].
...
Common to all screens (from v2 C): mobile first (375 → 768 → 1280), one H1, `<main>`, skip link, 3px pine focus ring, 44px targets, logical CSS for RTL, colour never the only signal, Gregorian calendar and Latin digits, German text through `<De>`. **SSR (v3.1): the Lovable project is TanStack Start with server rendering.** Every page is rendered on the server first. Never read `window`, `localStorage`, `sessionStorage`, `navigator` or now()-based values during render; do it in `useEffect` or event handlers. Realtime channels, the call timer, `window.open`, the intake POST and `crypto.randomUUID()` run only in the browser. Page titles and meta come from each route's `head()`, never `document.title`. **Demo notice (v3.1):** in the footer on every page, above "Call now" on `/ask` and in the Start panel on `/r/:id`: `common.footerDemo` "This is a demo. Please use test data only." The footer also links `/impressum` and `/datenschutz`. Pattern names refer to `component-reference-design` (`components.md`, `layouts.md`, `dashboards.md` §8 filters and §10 states).
...
- **Layout (anti-monotony, every section different):**
  ...
  5. **How Ask for me works:** horizontal numbered list with a thin connector: Find · Ask · We call · Answer. Optional (anti-slop): lead it with one real excerpt from a demo call (German line + translation in script-row style; the strings `landing.excerpt*` exist), so the section shows the product instead of a generic 4-step strip.
...
| District | popover, multi | distinct `district`: the German name in `<De>`, plus the script name `districts.<name>` in ru/uk/ar when it differs (the strings file transliterates: "Нойштадт", "نويشتات"); missing keys (live: Bühlau, Seidnitz) show only the German name | equals |
...
- **Checked by phone badge:** only when `last_checked_at` is set **and** `subcategory = 'demo'` (v3.1: every call this weekend is a browser role-play, so a badge on a real listing would be false; lift this rule when a real phone line exists). Pine-tint, phone icon, "Checked by phone on 27 Sep 2026" + one outcome line: `booked` "A slot was booked", `completed` "They answered our questions", `rejected` / `rejected_no_new_patients` "No free place at that time". An info button explains: "Our AI assistant phoned them for a parent. We only show the date and the kind of answer." Never who asked, never what was said. (DEF-047 fix, migration `20260926220000_checked_badge_ignore_tests.sql`: the trigger skips calls with `provider` test/verify/mock and requests whose `patient_name` contains "(E2E", "roleplay" or "test". Relay role-plays with an ordinary name (`provider = 'deepgram-browser'`) still set it, hence the demo-only rule above.)
- **States:** skeleton; not found "We can't find this place." + link to `/courses`; error + retry.
- **Optional:** JSON-LD `LocalBusiness` with the fields we have (Idea B asks for schema.org markup), emitted from the route's `head()` so it is server-rendered; never for `subcategory = 'demo'` (fictional business).
...
- **States:** skeleton; empty "No events in the next 5 weeks match your filters." + "Show all events" + "Suggest an event"; error + retry. Past events never show (query from today). An event that started earlier but still runs (`ends_at` ≥ today, e.g. the Zoo autumn holidays 10–25 Oct) is missed by this query; after the demo query `starts_at ≥ today OR ends_at ≥ today`.
...
4. **Questions for the new types:** child's age (number + years/months), start month (`<input type="month">`), time windows (as v2), preferred language of the course. Facts use the keys the relay reads: `child_age`, `start_month`, `child_birth_month`, `language_preference` (F.6). The app never asks for the child's name. Since the DEF-050 fix (26 Sep, [[RUN-025 Data round 2 and review sheets]]) workflow 04 itself drops child-name facts, notes and `missing` items (`/child.*name/i`); the client drop stays as a guard.
...
- needs_user / failed / no_answer / voicemail: as v2 C4 (the retry goes to `/ask?resource=…&type=…`). `failed` with **no** `calls` row means the brief failed before any call: show the Start panel (C7), not "The call didn't go through". `calls.result` and `summary_user` can be null (the seeded `/demo` rows): read them with `?.`. While `summary_user` is still null, show a muted "Writing the summary in your language…" instead of the English `summary_en` marked as the user's language. Under every summary: muted "Summary translated by our AI assistant. The German lines are what they said."
...
Redirect to the finished demo request (unchanged). The seeded Arabic pharmacy result `/r/00000000-0000-0000-0000-000000000002` is the RTL backup (see H). Note: `…0001` is an English doctor booking (Priya Sharma, seeded; `result` and `summary_user` are null), not the family story; optionally point `/demo` at the rehearsal's course request after the rehearsal.
...
| `--amber-text` / `--amber-tint` | #8A5A00 / #FBF0D9 | 39 100% 27% / 41 81% 92% | "needs your answer" (only on `/r/:id`) | 5.2 |
| `--destructive` brick / `--brick-tint` | #A63A2A / #F6E4DF | 8 60% 41% / 13 56% 92% | errors, rejected | 5.2 |
| `--border` | #D9D2C3 | 41 22% 81% | decorative lines, age ruler track | pine fill vs track 5.2 |
| `--input` | #8C8577 | 40 8% 51% | **all control outlines** (inputs, chips, checkboxes, segmented) | 3.3 paper, 3.6 card, 3.03 paper-deep (passes by 0.03: the Ask-for-me box, approval card and consent sit there), 3.1 pine-tint, 2.97 sand (fails). Optional darker value #7D776A (41 8% 45%): 4.1 paper, 3.7 paper-deep, 3.6 sand |
...
- **Type:** unchanged scale 14 / 17 / 21 / 26 / 33 / 41 (52 home H1, 36 mobile). Headings Fraunces 600 SOFT 50 (Latin); Source Serif 4 600 for Cyrillic headings (Fraunces has no Cyrillic: its Google Fonts subsets are latin, latin-ext, vietnamese); Noto Naskh Arabic 600 / Noto Sans Arabic for Arabic (18px, line-height 1.8). Body Source Sans 3 400/600 (has Cyrillic). Tabular numbers for dates, times, prices, counts. **Font loading (v3.1): self-hosted, no Google Fonts link.** Install npm packages and import them once in the root (`src/routes/__root.tsx` or `src/styles.css`): `@fontsource-variable/fraunces` (use its all-axes file if it includes the SOFT axis, check in `node_modules`; else `@fontsource/fraunces` 400 + 600 and drop SOFT), `@fontsource/source-sans-3` (400, 600), `@fontsource/source-serif-4` (600, Cyrillic headings), `@fontsource/noto-sans-arabic` (400, 600) and `@fontsource/noto-naskh-arabic` (600). Remove the fonts.googleapis.com / fonts.gstatic.com links from `__root.tsx` `head()` (today at line 73, without SOFT). Reason: loading fonts from Google sends every visitor's IP address to Google, which LG München I (20 Jan 2022, 3 O 17493/20) held unlawful without consent; with self-hosted fonts the site needs no consent banner. Axes we use: Fraunces opsz 9–144, wght 400–600, SOFT 0–100.
...
All seven must-fix items of [[Accessibility and RTL Review]] (§4) are built in, not left for a later clean-up. Items 1–6 are in the Lovable prompts. Item 7 (and the test-page half of item 4) is about the relay call page `services/voice-relay/public/index.html`, which Lovable does not build. The receptionist uses that page on stage, and "Start the call" now opens it, so it was fixed in G.0 step 5 (Claude Code, zero credits). **Done 26 Sep evening:** title "Dresden mit Kind · Call page", `#status` with `role="status"` and the dot `aria-hidden`, `lang="de"` on German lines, focus moves to "Hang up" when the socket opens and back to "Start call" on clean-up, plain messages for `NotAllowedError` / `NotFoundError` / socket closed before open, `#lines` without `aria-live` plus an h2 and `aria-labelledby`.
...
| 2 | F2 `dir` follows the UI language | `uiLang` (en, de, ru, uk, ar, tr) alone sets `<html lang dir>` (rtl only for ar). With SSR, `RootShell` in `src/routes/__root.tsx` renders `<html>` (today hard-coded `lang="en"`, line 86): keep the choice in a first-party cookie `dmk_ui_lang` (functional, no consent needed), read it on the server in the root route (`createIsomorphicFn` + `getCookie`) and render `lang`, `dir` and the strings from it, so Arabic arrives mirrored with no flash and no hydration mismatch. Fallback: server and first client render in `en`, switch in `useEffect`. Never read `localStorage` or `navigator` during render. `userLang` blocks get `lang` + `dir` from their language. Fonts keyed on `[lang\|=…]` of any element, not `:root` | P1b |
...
**New v3 items:** wrap the app in Radix `DirectionProvider dir={dir}` (arrow-key order in RadioGroup, ToggleGroup and DropdownMenu); the mobile menu Sheet opens from the end side (`side={dir === "rtl" ? "left" : "right"}`, Radix sides are physical); each language name is a `<bdi lang={code}>` so RTL names keep their order between " · " separators (live rows have `ar`, `fa`, `prs`, `ur`); `Intl.DisplayNames` falls back to the raw code when it has no name (rare live codes such as `gsg`, `ce`, `ti`); adjacent chips and touch targets keep an 8px gap; filter bar `role="toolbar"`, removable chips with translated `aria-label`, no focus jump on filter change; age ruler `aria-hidden` with the text next to it; date tiles as `<time datetime>`; language names always carry `lang` (`<span lang="ru">Русский</span>`); English fallback text carries `lang="en"` inside a Russian page; honeypot hidden from assistive tech; `html { scroll-padding-top: 5rem }` (F10); all interactive rows ≥ 44px (F25); test at 320px and 200% zoom (F28), Turkish and Ukrainian strings are ~30% longer.

---

## F. Data contract for Lovable (exact)

Source of truth: `supabase/migrations/*.sql`, checked against the live project on 26 Sep ([[RUN-022 UI plan v3 merged]]). Re-checked live on 26 Sep evening (v3.1): all 10 migrations applied; tables, columns, check constraints, policies, grants and the Realtime publication match this section; data per [[RUN-025 Data round 2 and review sheets]]. Lovable **never** creates or changes tables.

### F.1 What the app may do with the anon key (browser and SSR server; RLS re-checked live 26 Sep evening)
...
| `call_requests` | INSERT (`consent_ai_call = true`, `status = 'submitted'`, `call_brief_de` and `goal_de` null) + SELECT | demo mode: anyone with the public anon key can list **all** rows (names, phones, goals, facts), not only by link; so: no email field, demo notice "Please use test data only", `noindex` on `/r/*` |
| `calls`, `transcript_lines`, `events` | SELECT + Realtime | written by n8n / relay only |

**Server side (v3.1):** loaders run on the SSR server too, with the same anon/publishable key (`src/integrations/supabase/client.ts`), so RLS is identical there. Never import `@/integrations/supabase/client.server` (service-role key, bypasses RLS) or `auth-middleware.ts` from routes, components or `*.functions.ts`. No `createServerFn` or server routes that touch Supabase; the only server-side code allowed is reading the language cookie (E row 2). Anon cannot update or delete any row (no policies) and cannot read `suggestions` (no grant).

### F.2 `resources` (read)

`id` uuid · `category` `auslaenderbehoerde|doctor|pharmacy|bank|community|other|course|kita|school|library|playground|family_place` · `subcategory` text (54 values live, each with a `subcategories.<key>` string, incl. `german_for_children` from RUN-025; e.g. `kinderarzt`, `welcome_center`; **`demo`** = demo listing, show "Demo listing, not a real business") · `name` · `address` · `district` (Dresden Stadtteil) · `city` · `lat`, `lng` · `phone` (may be null) · `website` · `languages` text[] ISO codes, spoken / of instruction, **nullable** · `opening_hours` · `notes_en` · `source` `brave|firecrawl|manual|web_research|suggestion` · `source_url` · `retrieved_at` · `audience` `family|newcomer|both` · `activity_categories` text[] ⊆ `music, dance, sport, yoga, art, languages, stem, swimming, theatre, nature, parent_baby, parent_meetup, library, school_kita, family_cafe, playground` · `age_min_years`, `age_max_years` numeric 0–18 (null = not listed; 0.5 = 6 months) · `price_type` `free|per_session|subscription|trial_available|unknown` · `format` `recurring_course|workshop|one_off|community_group|place|service` or null · `description_en` ≤ 600 · `i18n` `{"de":{"description":"…"},"ru":{…},"uk":{…},"ar":{…},"tr":{…}}` · `last_checked_at` · `last_check_outcome` `booked|completed|rejected|rejected_no_new_patients` (set only by a trigger).
...
Live (v3.1, RUN-025): 162 rows (73 `family`, 19 `both`, 70 `newcomer`); `/courses` categories 51 rows (38 courses, 4 playgrounds, 9 family places); `/services` 94 rows (84 newcomer services + 7 Kitas + 3 schools); language codes go beyond the 14 content languages (e.g. `ce`, `gsg`, `ka`, `ti`, `ur`); no "checked by phone" badge set yet.
...
| `doctor` | services → Doctors | `doctor_appointment`; for `subcategory = 'kinderarzt'` use `other_call` ("Ask if they take new patients") until a child-patient mode exists: the doctor template books under `patient_name`, which is the parent (the guide `kinderarzt-u-untersuchungen` keeps `ask_task_type = doctor_appointment` in the DB; a data change is the owner's call) |
...
`POST ${INTAKE_URL}` (default `https://arahmandeaxo.app.n8n.cloud/webhook/task-intake`), `Content-Type: application/json`. The workflow accepts only these origins: `*.lovable.app`, `*.lovableproject.com`, `localhost:8787`, `127.0.0.1:8787`. A custom domain gets 403 `origin_not_allowed` until it is added to that list ("Check Origin + Size", lines 5–10) and to the relay's `ALLOWED_ORIGINS` for P13. Call it only from the browser (an event handler), never from a loader or server function: those send no Origin header. Nothing is stored; executions are not saved.
...
**Client rule (prefill wins):** after each response: `draft.task_type = prefill.task_type ?? draft.task_type`; copy every non-empty `prefill.organisation` field over `draft.organisation`; keep `resource_id`; drop `missing` items whose `field` starts with `organisation` when name and phone are known; drop `missing` items for `child_first_name` / `child_name` (never ask for the child's name). The live server does all five itself (the first four since 13:09, the child-name filter since the DEF-050 fix in RUN-025); the client keeps them as a guard.
...
await supabase.from("call_requests").insert({
  ...
  user_email: null,                            // no email field: no workflow sends email, and demo RLS lets anyone read call_requests
...
`call_requests.status`: `submitted` → `briefed` (n8n 01, ~10 s; or `failed` straight from `submitted` if the brief fails) → `calling` → `booked | completed | needs_user | rejected | failed`. `user_language` allows `en, ar, tr, uk, ru, fa, prs, hi, es, fr, pl, vi, zh, de`.
...
- **calls:** `outcome` `booked|completed|needs_user|rejected|rejected_no_new_patients|no_answer|voicemail|failed` · `booked_slot` (UTC) · `bring_items` text[] · `bring_items_user` jsonb array `[{"de":"Hausschuhe","meaning_user":"сменная обувь"}]` (n8n 06 "Parse Result"; look up with `.find(i => i.de === word)`) · `summary_en` · `summary_user` · `result` jsonb:
...
- **transcript_lines:** `id` (order), `call_id`, `created_at`, `speaker` `agent|practice|system`, `text_de`, `text_user` (later, by UPDATE), `text_en`. **Live subtitles only for the other side:** since migration `20260926210000_credits_translate_practice_only.sql` (n8n credits) the trigger sends only `practice` lines to n8n 05. `agent` lines get no live `text_user`; the parent already approved what the assistant says. Workflow 06 translates the agent lines (and any practice line that missed its live subtitle, max 30 per call) in one batch when the outcome is set, about 15 s later (live since RUN-024, 13:25, and in the export: nodes "Get Untranslated Lines" and "Save Line Subtitles"). So agent rows have no subtitle during the call and fill in by UPDATE afterwards. The UI shows no placeholder ("Translating…", "Subtitle not available") under agent rows, shows `text_user` when it arrives, and never announces these late subtitles (P9 step 4).
...
- **One request, several calls:** every start on the relay page inserts a new `calls` row for the same request, so a request can have an old finished call plus a newer unfinished one (seen on the demo request `…0001` on 26 Sep; reset since, but the relay page and `server.js:358` still default to that ID, so it can happen again). The app shows the latest call that is still running (`ended_at` null) or has an `outcome`; calls that ended without an outcome (aborted tests) are skipped; if nothing qualifies, the latest call (P9).
- **Badge:** when `calls.outcome` becomes `booked|completed|rejected|rejected_no_new_patients` and the request has `resource_id`, a trigger sets `resources.last_checked_at = now()` and `last_check_outcome`, except for calls with `provider` test/verify/mock or `patient_name` containing "(E2E", "roleplay" or "test" (migration `20260926220000`); the UI shows the badge only for `subcategory = 'demo'` (C3). The app only reads it (the provider page refetches on open; no Realtime on `resources`).
...
1. **Lovable GitHub sync: done 26 Sep.** Repo `AbdulrahmanBUW/hallotermin-paper` (local clone `../hallotermin-paper`). The project is **TanStack Start with SSR**: file routes in `src/routes` (`p.$id.tsx` = `/p/:id`), the only layout is `src/routes/__root.tsx`, Tailwind v4 tokens in `src/styles.css`; no `index.html`, no `tailwind.config`, no react-router. Push only working commits to `main`, never force-push (`AGENTS.md`). Strings, types and static files are fixed there by Claude Code at zero credits.
2. **Strings: done 26 Sep.** `src/i18n/strings/{en,de,ru,uk,ar,tr}.json`, split from `docs/i18n/ui-strings.json` (source of truth; split command in `docs/i18n/README.md`, "Per-language files"): **796 keys per language**, including `districts.Bühlau` / `districts.Seidnitz` and the legal keys `common.footerImpressum`, `common.footerPrivacy`, `legal.impressumTitle`, `legal.privacyTitle`, `legal.germanBinding`; 183 RU/UK/AR translation fixes were applied on 26 Sep. Edge-case keys the prompts use: `taxonomy.age_<band>` (Arabic writes "3-6" with a hyphen, because an en dash is shown reversed in RTL), `directory.fromOneYear` / `upToOneYear`, `directory.agesMissing`, `filters.triggerCount`, `filters.countEvents(One)`, `subcategories.*` (54), `districts.*`. Native speakers (Anastasia's communities) still review ru, uk, ar ([[DEC-003 Merged concept]]).
3. **Supabase types and dependency: done 26 Sep.** `src/integrations/supabase/types.ts` regenerated with `npx supabase gen types typescript --linked --schema public` (8 tables; the 25 Sep file had 4, so Lovable would have seen "missing tables") and `@supabase/supabase-js` ^2.117.1 re-added to `package.json` (the 25 Sep merge "Lovable update" had dropped it). After these pushes Lovable showed "Build unsuccessful" (probably `bun.lock`); P1b step 0 asks Lovable to fix the build first.
4. **Knowledge v3.1: done 26 Sep** (G.1, pasted and saved in Lovable).
5. **Env / config:** `src/config.ts` holds `INTAKE_URL` and `RELAY_URL` with public defaults (P1b creates it). When the relay host is known, change the `RELAY_URL` default in `src/config.ts` via GitHub (zero credits; don't edit `.env`, Lovable's Supabase connection writes it; a `wss://` value is converted to `https://`). On the relay host set `ALLOWED_ORIGINS` = the relay's own https origin + the published app origin (otherwise the relay page gets 403), and once the app URL is fixed `ALLOWED_ORIGIN_SUFFIXES=` (empty). These are public URLs, not secrets.
6. **Data (live 26 Sep, 15:10, [[RUN-025 Data round 2 and review sheets]]):** 162 resources; `/courses` 51 rows (38 courses, 4 playgrounds, 9 family places), `/communities` 25, `/services` 94; 92 family rows (77 with a phone, 46 with an age range; all with `i18n` in ar, de, ru, tr, uk); 44 family events 27 Sep – 29 Nov (27 in the next 35 days; languages de, es, fr; free 30 / paid 10 / unknown 4); 8 guides with `i18n` in ar, de, ru, tr, uk. The demo listing **"Olgas Musikstudio"** exists (`ae0ae23b-0dd7-4f82-adf8-6fd78aa3456d`, `subcategory = 'demo'`, `+49 351 0000000`, Neustadt, music, 3–6, ru + de, `trial_available`, no lat/lng, no website). The `/demo` request `…0001` is reset (status `booked`, only the mock call). The prompts also work on empty tables (empty states).
7. **Relay call page, accessibility must-fix 7 (+ the test-page half of 4): done 26 Sep** in `services/voice-relay/public/index.html`: German `<li>` lines get `lang="de"` (F5); focus "Hang up" after `ws.onopen`, back to "Start call" in `cleanup()` (F11); `#status` has `role="status"` and the dot `aria-hidden="true"` (F12); plain messages of [[Accessibility and RTL Review]] F14 for `NotAllowedError` / `NotFoundError` / socket closed before open instead of `err.message` (F14); `#lines` without `aria-live`, with a real heading + `aria-labelledby` (F3, F26). Check: keyboard-only start and hang-up, NVDA reads German lines with a German voice.
...
### G.1 Knowledge v3.1 (pasted and saved in Lovable → Project settings → Knowledge, 26 Sep; replaces v3; 9,609 characters)

```text
PRODUCT
Dresden mit Kind: the family hub for international parents in their first years in Dresden. Pillars: Courses & activities, Events, Bilingual communities, Health & services, Library (guides). Feature "Ask for me": from a provider or guide page the parent asks our AI assistant to phone the place in German (free spot, trial lesson, Kita place, appointment). It says it is an AI in its first sentence, calls for the parent, and the answer comes back in the parent's language.
Loop: Discover -> Understand -> Act -> Result. Users: parents with little German, mostly on phones.

STACK (do not change)
TanStack Start with SSR. File routes in src/routes (p.$id.tsx = /p/:id; __root.tsx is the only layout). <Link to="/p/$id" params>, query via validateSearch + Route.useSearch(), redirect() in beforeLoad, titles/meta via head(). Never react-router-dom, index.html or document.title. Tailwind v4: tokens in src/styles.css (@theme inline), no tailwind.config.
SSR: window, document, localStorage, navigator, matchMedia, crypto, WebSocket only in effects or event handlers, never during render. /ask and /r/$id use ssr: false. Read-only pages may load data in route loaders with the same anon client.

LANGUAGES
UI languages: en, de, ru, uk, ar (RTL), tr. Strings in src/i18n/strings/<lang>.json (from GitHub; never rewrite or translate them), shape {"<screen>":{"<key>":"text"}}; t("screen.key", vars) = strings[ui][screen][key] ?? strings.en[screen][key]; replace {name} placeholders; never show raw keys. Look up the existing key in en.json first; if one is really missing, add it to en.json only and list it in chat.
Keep two ideas apart: uiLang (one of the 6; cookie dmk_ui_lang read by the root route on the server; sets <html lang dir> in __root.tsx, rtl only for ar) and userLang (language of a request or text; en ar tr uk ru fa prs hi es fr pl vi zh de). Content blocks get lang={code} and dir from the language (rtl for ar, fa, prs). dir="auto" only for text the user typed.
German text always via <De>: <span lang="de" dir="ltr" class="de">.
Content: <table>.i18n[ui].<field> ?? the English column; on fallback a muted t("directory.inEnglish") and lang="en" on the text.
Dates, times, prices only via src/lib/format.ts (Gregorian, Latin digits, h23, Europe/Berlin). Language names via Intl.DisplayNames([code]) inside lang={code}. No flags.

ARCHITECTURE (do not change)
- Backend = our own Supabase project (ref ycyrtlzympxzlfcocazh), already connected. Never enable Lovable Cloud. Schema lives in SQL migrations in another repo: NEVER create/alter/drop tables, columns, policies, triggers, functions or buckets, never propose a migration, never apply a security-scan fix as SQL. If something is missing, say so in chat and stop.
- Only the anon key via import { supabase } from "@/integrations/supabase/client". Never import client.server.ts, supabaseAdmin or auth-middleware. No secrets in code.
- The browser only: READS resources, family_events, guides; INSERTS suggestions (never .select() after it); POSTs to INTAKE_URL from the browser (never from a loader or server function: the webhook checks the Origin); INSERTS one call_requests row; READS + Realtime call_requests, calls, transcript_lines, events; opens ${RELAY_URL}/?request=<id> in a new tab. INTAKE_URL, RELAY_URL, RELAY_URL_LOCAL come from src/config.ts (public URLs). No Edge Functions, no auth, no other webhooks.

HARD RULES
- Never store the parent's free text (React state only, cleared after insert). Never put personal data in URLs; pass text between pages in router history state. No email field.
- Child data: age (or birth month) and start month only. Never ask for a child's name. Never ask about or show health, allergies or development. If sensitive_removed=true show t("ask.sensitiveRemoved").
...
- "Checked by phone on <date>" only when subcategory = 'demo' and last_checked_at is set (all calls this weekend are browser role-plays).

DATA CONTRACT (exact lowercase values)
resources (read): id, category auslaenderbehoerde|doctor|pharmacy|bank|community|other|course|kita|school|library|playground|family_place, subcategory ('demo' = demo listing: tag "Demo listing, not a real business"), name, address, district, phone, website, languages text[] (ISO, may be null), opening_hours, notes_en, audience family|newcomer|both, activity_categories text[] (16 keys, see taxonomy), age_min_years, age_max_years (0-18, null = not listed), price_type free|per_session|subscription|trial_available|unknown, format recurring_course|workshop|one_off|community_group|place|service|null, description_en, i18n, lat, lng, source, source_url, retrieved_at, last_checked_at, last_check_outcome booked|completed|rejected|rejected_no_new_patients.
...
call_requests (insert with consent_ai_call=true; never send status, goal_de, call_brief_de): id (crypto.randomUUID()), task_type doctor_appointment|authority_appointment|landlord_request|contract_question|bank_enquiry|pharmacy_question|restaurant_booking|other_call|course_enquiry|kita_enquiry, user_language, patient_name (the parent, Latin letters, not empty), user_email null, practice_name, practice_phone (not empty), organisation_category, resource_id, goal_user (<=200), reason_category (doctor only, required), insurance_type gkv|pkv|other|null, insurance_name, has_referral false, is_new_patient true, time_windows [{date,from,to}], constraints {}, allowed_facts [{key,label,value}] as real JSON. status submitted|briefed|calling|booked|completed|needs_user|rejected|failed.
calls (read): id, request_id, created_at, started_at, ended_at, outcome booked|completed|needs_user|rejected|rejected_no_new_patients|no_answer|voicemail|failed, booked_slot, bring_items, bring_items_user [{de,meaning_user}], result jsonb, summary_en, summary_user.
transcript_lines (read, Realtime INSERT + UPDATE): id, call_id, created_at, speaker agent|practice|system, text_de, text_user (arrives later: live for practice lines, after the call for agent lines), text_en.
events (read): id, created_at, request_id, type, payload.

ROUTES
/ · /courses · /events · /communities · /services (?group=kita_school|doctors|pharmacies|offices|banks|advice) · /library · /guides/:slug · /p/:id · /ask (?resource=&type=&guide=) · /r/:id · /suggest (?kind=&resource=) · /impressum · /datenschutz · /demo -> /r/00000000-0000-0000-0000-000000000001 · /find -> /services · /new -> /ask · 404

DESIGN SYSTEM "Bilingual paper v3"
Colours: paper #F7F4EC bg; card #FFFDF8; paper-deep #EFE9DC (approval slip, Ask-for-me box, one full-bleed band); sand #F2E6D3 (event date tiles and the home age band only, never under inputs); ink #1C2420; ink-muted #5B635E; pine #1F5C4A primary (hover #174A3B, white text); pine-tint #E6EEE9 (selected, assistant lines, checked badge); amber #D98E04 only for the live dot, with a 2px #8A5A00 ring and the word "Live"; amber-text #8A5A00 on amber-tint #FBF0D9 = needs your answer; brick #A63A2A on brick-tint #F6E4DF = errors; border #D9D2C3 decorative only; control outlines #8C8577; focus ring pine 3px, offset 2px.
Fonts (self-hosted @fontsource, never Google Fonts): headings + wordmark Fraunces 600 with "SOFT" 50; German text Fraunces 400. Body/UI Source Sans 3 400/600. Cyrillic headings Source Serif 4 600. Arabic: Noto Naskh Arabic 600 + Noto Sans Arabic 400/600, 18px, line-height 1.8, no italics or letter-spacing. Fonts keyed on [lang|=..] of any element.
...
Motion <=200ms, off under prefers-reduced-motion; only the live dot loops.
...
### G.2 Build prompts (demo path first: P1b, P2, P3, P7–P11, P14, then P4–P6, P12; P13 optional)

> Send each in **Agent mode** exactly as written, one at a time. Check the preview, then publish (free). One follow-up fix at most; after that fix it in the repo with Claude Code (GitHub sync is on; never force-push). Order = demo path first: P1b (sent) → P2 → P3 → P7 → P8 → P9 → P10 → P11 → publish → P14, then P4 → P5 → P6 → P12 (P13 optional). P7–P10 need the intake and relay. The project is TanStack Start with SSR (G.0 step 1); the prompts below are written for it.

#### P1b: Rename the shell, pillar navigation, 6 UI languages, RTL, strings, format helpers (no DB) — sent 26 Sep, about 16:00

Sent with an extra first line, step 0, asking Lovable to fix the "Build unsuccessful" state after the GitHub pushes (probably `bun.lock`) before anything else; the block below is the rest of the message as sent.

```text
Turn the existing shell "HalloTermin Paper" into "Dresden mit Kind". This project is TanStack Start with SSR: file routes in src/routes, the only layout is src/routes/__root.tsx (RootShell renders <html>), Tailwind v4 tokens live in src/styles.css; there is no index.html, no tailwind.config and no react-router-dom. Follow Knowledge (STACK, LANGUAGES, DESIGN SYSTEM, RTL, BANS). No database code in this step. Keep src/routes/r.$id.tsx and src/routes/demo.tsx working.

1. Brand: wordmark "Dresden mit Kind" (lang="de") in Fraunces 600 with font-variation-settings "SOFT" 50, "Dresden" in ink and "mit Kind" in pine, linking to /. Replace every "HalloTermin" with "Dresden mit Kind", also in every head() title, description and og tag, the root meta author and aria-labels. The calling feature is called "Ask for me". Every route sets its title in head() as "<page> · Dresden mit Kind" (never document.title).
2. Fonts, self-hosted (no requests to Google Fonts: German courts treat that as a GDPR breach without consent). Add @fontsource-variable/fraunces, @fontsource-variable/source-sans-3, @fontsource/source-serif-4, @fontsource/noto-naskh-arabic, @fontsource/noto-sans-arabic. At the top of src/styles.css: @import "@fontsource-variable/fraunces/full.css" (includes the SOFT axis); @import "@fontsource-variable/source-sans-3"; @import "@fontsource/source-serif-4/600.css"; @import "@fontsource/noto-naskh-arabic/600.css"; @import "@fontsource/noto-sans-arabic/400.css"; @import "@fontsource/noto-sans-arabic/600.css". Remove the Google Fonts stylesheet and both preconnect links from the head() of __root.tsx. --font-display: "Fraunces Variable", serif; --font-sans: "Source Sans 3 Variable", sans-serif. Headings Fraunces 600 "SOFT" 50; class .de = Fraunces 400. CSS keyed on any element, not only :root: [lang|="ar"],[lang|="fa"] { font-family: "Noto Sans Arabic", sans-serif; font-size: 1.0625em; line-height: 1.8; letter-spacing: 0; font-style: normal } and their headings "Noto Naskh Arabic"; headings inside [lang|="ru"], [lang|="uk"] use "Source Serif 4".
3. Tokens in src/styles.css: add to :root --sand: 37 54% 89%; --paper-deep: 41 37% 90%; --live-ring: 39 100% 27%; and register them in the @theme inline block as --color-sand: hsl(var(--sand)); --color-paper-deep: hsl(var(--paper-deep)); --color-live-ring: hsl(var(--live-ring)). All inputs, checkboxes, chips and segmented controls use the --input colour for their outline, never --border.
4. i18n, SSR-safe. The strings already exist in src/i18n/strings/{en,de,ru,uk,ar,tr}.json (pushed from GitHub, 796 keys each): use them; never rewrite, re-format or re-translate them. UI languages: en English, de Deutsch, ru Русский, uk Українська, ar العربية, tr Türkçe.
   a) src/i18n/lang.ts: getUiLangFn = createServerFn({ method: "GET" }) from @tanstack/react-start. Its handler returns the cookie dmk_ui_lang (getCookie from @tanstack/react-start/server) if it is one of the 6, else the first of the 6 in the Accept-Language header (getRequestHeader), else "en". It reads nothing else: no Supabase, no secrets.
   b) __root.tsx: loader: async () => { const ui = await getUiLangFn(); const strings = ui === "en" ? en : (await import(`../i18n/strings/${ui}.json`)).default; return { ui, strings }; } with en.json imported statically.
   c) src/i18n/index.tsx: a LanguageProvider fed from Route.useLoaderData() in RootComponent, and useT(): t("screen.key", vars) = strings?.[screen]?.[key] ?? en[screen]?.[key] ?? "" with {name} placeholders replaced; never render the raw key.
   d) setUiLang(code), only in the click handler: document.cookie = "dmk_ui_lang=<code>; path=/; max-age=31536000; SameSite=Lax"; set document.documentElement.lang and .dir at once; then router.invalidate() so the root loader loads the new strings.
   e) Never read localStorage, navigator, window or document during render.
5. RootShell renders <html lang={ui} dir={ui === "ar" ? "rtl" : "ltr"}> from Route.useLoaderData(), so the server HTML is already right (no flash, no hydration mismatch). Create two helpers: <De>{text}</De> renders <span lang="de" dir="ltr" className="de">, and <UserText lang={code}> renders its children with lang={code === "prs" ? "fa-AF" : code} and dir="rtl" for ar, fa, prs, else "ltr".
6. Header (solid paper, 1px bottom border, not sticky, not blurred): wordmark at the start; pillar links t("nav.courses") -> /courses, t("nav.events") -> /events, t("nav.communities") -> /communities, t("nav.services") -> /services, t("nav.library") -> /library; at the end an outline button t("nav.ask") -> /ask and the language button (lucide "Languages" icon + the current language's native name inside <span lang=code>, aria-label t("nav.language")). The language button opens a shadcn DropdownMenu with the 6 languages: native name (with lang) + the name in the current UI language in muted text; a check icon on the current one; no flags. Below 1024px the pillar links move into a Sheet opened by t("nav.menu"); the language button and "Ask for me" stay visible. Current page: <Link activeProps> with pine text + 2px underline + aria-current="page".
7. Routes as TanStack files in src/routes, each a placeholder with one H1 and a head() title for now: courses.tsx, events.tsx, communities.tsx, services.tsx, library.tsx, guides.$slug.tsx, p.$id.tsx, ask.tsx, suggest.tsx, impressum.tsx (H1 t("legal.impressumTitle")), datenschutz.tsx (H1 t("legal.privacyTitle")). Keep r.$id.tsx and demo.tsx (-> /r/00000000-0000-0000-0000-000000000001). find.tsx redirects to /services and new.tsx (a page today) redirects to /ask, both with throw redirect() in beforeLoad. 404: keep the notFoundComponent in __root.tsx with t("common.notFoundTitle") and the link t("common.notFoundLink"); the errorComponent uses t("errors.loadFailed") and t("common.tryAgain").
8. src/lib/format.ts (pure functions, they run on the server and in the browser): loc(lang) = `${lang === "prs" ? "fa-AF" : lang}-u-ca-gregory-nu-latn` (works for the 6 UI languages and for content languages such as fa and prs, which would otherwise get the Persian calendar); formatDate(iso, ui), formatDateTime(iso, ui) (weekday, day, month, time), formatTime, formatRange(startIso, endIso, ui) via Intl.DateTimeFormat#formatRange, all with hourCycle "h23" and timeZone "Europe/Berlin"; formatPrice(eur, ui) via Intl.NumberFormat currency EUR; startOfTodayBerlin() returns an ISO string.
...
11. Footer, minimal, muted: t("common.footerAbout") / t("common.footerDemo") / t("common.footerListings") / t("common.footerEmergency"), then two small links: t("common.footerImpressum") -> /impressum and t("common.footerPrivacy") -> /datenschutz. No other links.
Do not build page content yet.
```

**Check it worked:** the wordmark reads "Dresden mit Kind" with softer serif ends; the five pillar links and "Ask for me" are in the header. Switch to العربية: the layout mirrors, digits stay 0–9. Switch to Русский: layout stays left-to-right and headings render in a serif (Source Serif 4), not a fallback. Reload keeps the language. `/find` lands on `/services`, `/new` on `/ask`, `/demo` on the demo call. No flags, no purple. After choosing العربية, view source of `/`: the server HTML already has `<html lang="ar" dir="rtl">`, and the console shows no hydration warning. `/impressum` and `/datenschutz` show their placeholder H1; the footer has both links.

#### P2: Directory building blocks + `/courses`

```text
Build the shared directory pieces and the /courses page. Read-only from resources. Do NOT create or change tables. All text via t() (screens "directory", "filters", "taxonomy"). src/integrations/supabase/types.ts is current (8 tables); if a table or column seems missing, stop and tell me (never a migration).
...
- CheckedBadge({at, outcome, subcategory}): render only if at is set and subcategory === "demo" (Knowledge: all calls this weekend are role-plays). Pine-tint background, pine text, lucide PhoneCall icon, t("directory.checkedOn",{date}) + one line by outcome (booked -> checked_booked, completed -> checked_completed, rejected or rejected_no_new_patients -> checked_rejected). A small info button opens a Popover with t("directory.checkedInfo").
- SourceLine({url, retrievedAt}): muted text t("directory.source",{date: formatDate(retrievedAt, uiLang)}) with the host name of url as a link (new tab, rel noopener).
- ListingRow(r): an <li> in a list with 1px dividers (no card grid). Contents: name as an <h3> with <Link to="/p/$id" params={{ id }}> (lang="de"); category label t(`taxonomy.cat_${category}`) + activity icons with t(`taxonomy.activity_${a}`); AgeRuler; LangList; district as <De>{district}</De> (in ru, uk and ar add the muted script name t(`districts.${district}`) when it is not "" and differs; t() returns "" for unknown districts); price label (taxonomy.price_<price_type>); CheckedBadge; if r.subcategory === "demo" a tag t("directory.demoListing"). Action: if TASK_FOR_CATEGORY[r.category] is not null, a text <Link to="/ask" search={{ resource: id, type: task }}> t("directory.askForMe"). If r.phone is empty, a muted line t("directory.noPhone") (the button still works; never invent a number).
...
5. All filters live in the URL query: validateSearch on the route (zod, every param an optional string), read with Route.useSearch(), change with navigate({ search: (prev) => ({ ...prev, act: "music,dance" }), replace: true, resetScroll: false }); example ?age=3-6&act=music,dance&lang=ru&district=Neustadt&price=trial_available&format=recurring_course. Filter in the component, not in the loader.
...
D. src/routes/courses.tsx: H1 t("directory.coursesTitle"), intro t("directory.coursesIntro"). The route loader runs this one query with the anon client (SSR; RLS applies) and returns the rows; pendingComponent = the skeleton, errorComponent = the error state:
...
Filter in the browser: age (matchesAge for rows with ages; rows where hasAges is false are NOT mixed in: when an age band is chosen, put them in a closed <details> at the end of the list with the summary t("directory.agesMissing",{count})), activity (overlap), language (overlap; rows with null languages hidden while a language is chosen), district, price, format. A small segmented control (?view=all|courses|places): t("directory.viewAll") · t("directory.viewCourses") (category course) · t("directory.viewPlaces") (playground, family_place).
States: 6 skeleton rows (aria-busy, animate-pulse off under reduced motion); empty: t("directory.empty") + a button that removes the most recent filter (t("directory.emptyTryWithout",{name})) + t("directory.emptyAllAges") + link t("directory.suggest") -> /suggest?kind=place; error: role="alert" t("directory.error") + t("common.tryAgain") (router.invalidate()).
head(): title "Courses & activities · Dresden mit Kind", description "Music, sport, art, languages and more for children in Dresden. Filter by age and language.", og:title, og:description.
```

**Check it worked:** `/courses?age=3-6&act=music` filters the list and survives a reload; removing a chip updates "N places" without moving focus; at 375px the age chips stay visible and "Filters" opens a sheet; in العربية the rows mirror and the age ruler fills from the right. `/courses` has 51 rows; `?age=3-6&act=music&lang=ru&district=Neustadt` leaves one row, "Olgas Musikstudio", tagged "Demo listing, not a real business".

#### P3: Provider page `/p/:id` with "Ask for me"

```text
Build src/routes/p.$id.tsx. Read-only (resources, family_events). No table changes. Reuse AgeRuler, LangList, CheckedBadge, SourceLine, TASK_FOR_CATEGORY from P2. The route loader checks params.id against the UUID pattern (else throw notFound()), runs both queries below with the anon client and throws notFound() when the resource is null.
...
5. CheckedBadge.
6. t("provider.upcoming"): up to 5 events as compact rows (sand date tile with weekday/day/month in <time dateTime>, title, time) linking with <Link to="/events" hash={id}>. Hide if none.
7. SourceLine; link t("provider.report") -> <Link to="/suggest" search={{ kind: "correction", resource: id }}> (hidden for demo listings); if lat and lng: text link t("provider.map") -> https://www.openstreetmap.org/?mlat=<lat>&mlon=<lng>#map=17/<lat>/<lng> (no embedded map).
Aside (paper-deep background, radius 10, 1px border, no shadow), only if TASK_FOR_CATEGORY[category] is not null: heading t("provider.askTitle"); text t("provider.askText",{place:name}); primary button t(`provider.ask_${task}`) as <Link to="/ask" search={{ resource: id, type: task }}>; if phone is empty, muted t("provider.noPhoneAsk") above the button. Under it "Contact": phone as a tel: link inside <bdi dir="ltr">, website link (new tab, rel noopener).
head({ loaderData }): title "<name> · Dresden mit Kind", description = description_en cut to 160 characters, og:title, og:description; for subcategory "demo" add { name: "robots", content: "noindex" } and no JSON-LD; for all others a JSON-LD LocalBusiness in head() scripts with only name, address, telephone, url.
States: pendingComponent skeleton; notFoundComponent t("provider.notFound") + link to /courses; errorComponent + t("common.tryAgain").
```
...
```text
Build /events in src/routes/events.tsx; the route loader computes from/to and runs the query (SSR). Read-only from the table family_events (NOT "events"). No table changes. Reuse the age chips, language popover, AgeRuler and LangList.
...
Filters: age chips (same matchesAge rule), Language popover (codes present), a checkbox chip t("filters.freeOnly") meaning price_type === "free". URL query ?age=&lang=&free=1 via validateSearch + Route.useSearch(). Result count t("filters.countEvents",{count}) (t("filters.countEventsOne") for 1) in role="status".
...
- links: t("events.organiser") -> url ?? source_url (new tab), t("events.calendar") -> .ics built in the click handler with src/lib/ics.ts (from P10; create it if missing: UTC DTSTART/DTEND, all_day -> VALUE=DATE, CRLF, escaping), t("events.place") -> /p/<resource_id> if set;
- muted line t("events.verify",{date: formatDate(retrieved_at, uiLang)}).
States: skeleton; empty t("events.empty") + t("events.showAll") + link t("events.suggest") -> /suggest?kind=event; error + retry.
head(): title "Events · Dresden mit Kind" + a one-sentence description.
```
...
- Each section ends with a link t("directory.eventsInLanguage",{language}) -> /events?lang=<code>, only if a family_events row in the next 35 days lists that language (load those languages once in the loader).
...
- Search input (label t("directory.search")), a segmented control (Radix ToggleGroup, single; URL ?group= with exactly these ids via validateSearch): all (default, no param) t("directory.groupAll") · kita_school t("directory.groupKita") (kita, school) · doctors t("directory.groupDoctors") (doctor; subcategory kinderarzt first, labelled t("subcategories.kinderarzt")) · pharmacies t("directory.groupPharmacies") · offices t("directory.groupOffices") (auslaenderbehoerde) · banks t("directory.groupBanks") · advice t("directory.groupAdvice") (community with audience "newcomer", and other). A district select (?district=).
- Rows = ListingRow, plus the subcategory in plain words from t(`subcategories.${subcategory}`) (e.g. hausarzt "Family doctor (Hausarzt)", kinderarzt "Children's doctor (Kinderarzt)", welcome_center "Welcome centre", migrant_counselling "Migration counselling"; 54 values in all 6 languages); if the key is missing, underscores to spaces.
- Under the list: t("directory.notVerified"). On this page also show t("common.footerEmergency") with 112 and 116117 wrapped in tel: links.
- Update src/routes/find.tsx: validateSearch { category?: string }; beforeLoad throws redirect({ to: "/services", search: { group } }) with kita/school -> kita_school, doctor -> doctors, pharmacy -> pharmacies, auslaenderbehoerde -> offices, bank -> banks, community/other -> advice; otherwise no group.
States as in P2. Both pages load their rows in the route loader (SSR); head() titles "Communities · Dresden mit Kind" / "Health & services · Dresden mit Kind".
```

**Check it worked:** `/services` lists the 84 newcomer places plus the seeded Kitas and schools (94 rows at 15:10: 84 newcomer rows, 7 Kitas, 3 schools; the 13 family communities stay on `/communities`, which has 25 rows); "Doctors" shows children's doctors first; "Advice" shows welcome centres and counselling; the Ask-for-me buttons open `/ask` with the right `type`. `/communities` shows one section per language plus "Languages not listed"; in Russian UI the Русский section comes first.

#### P6: `/library` and `/guides/:slug`

```text
Build /library and /guides/$slug. Read-only from guides. No table changes.
/library: H1 t("library.title"), intro t("library.intro"). Data (route loader, SSR): supabase.from("guides").select("slug,title_en,summary_en,i18n,category,audience,ask_task_type,updated_at").order("title_en").
...
/guides/$slug (src/routes/guides.$slug.tsx; only the P1b placeholder exists, there is no older layout): loader supabase.from("guides").select("slug,title_en,summary_en,checklist,content_md,sources,updated_at,i18n,category,ask_task_type").eq("slug", params.slug).maybeSingle(); null -> throw notFound() with t("guides.notFound"). Layout: H1 title, summary (21px), checklist as checkboxes (legend t("guides.checklistLabel")), content_md, then t("guides.sourcesTitle") with each link + t("guides.checkedOn",{date}) and t("guides.rulesChange"). Use i18n[uiLang] for title, summary and checklist when present, else English with a muted t("directory.inEnglish"). content_md stays English (add react-markdown, no raw HTML) with lang="en" and a muted t("guides.inEnglish") above it when the UI is not English. Checklist ticks in localStorage "dmk_guide_<slug>" (try/catch), read in useEffect after mount (the server renders them unticked). head(): "<title_en> · Dresden mit Kind", description summary_en.
...
3. H1: t(`ask.title_${task_type}`) for course_enquiry and kita_enquiry, else t("ask.title_default"). Helper with the matching example (t("ask.example_course_enquiry") / t("ask.example_kita_enquiry")) or t("intake.helper") + t("intake.helperExample") for other types.
4. Tell box: labelled textarea (4 rows, maxLength 600, counter "{count} / {max}", dir="auto"). If the page was opened with history state { text } (from the home box), put that text in: declare module "@tanstack/react-router" { interface HistoryState { text?: string } } and read useLocation({ select: (l) => l.state.text }). Chips that append a sentence in the UI language: course_enquiry -> t("ask.chipFreeSpot"), t("ask.chipTrial"), t("ask.chipWaiting"), t("ask.chipDays"); kita_enquiry -> t("ask.chipFromMonth"), t("ask.chipKitaWaiting"), t("ask.chipVisit"); other types -> t("intake.chipDoctor"), t("intake.chipAuthority"), t("intake.chipLandlord"), t("intake.chipPharmacy"), t("intake.chipBank"), t("intake.chipRestaurant"), each appending the matching intake.starter* text.
5. Continue: if empty, inline error t("intake.errorEmpty"). Else POST INTAKE_URL (src/config.ts) with fetch from the browser inside this click handler (never from a loader or server function: the webhook accepts only the browser Origin *.lovable.app / *.lovableproject.com), header Content-Type: application/json and exactly this body:
   { "text": <text>, "ui_lang": <uiLang>, "draft": null, "answers": null, "round": 1, "prefill": <prefill> }
   20 s timeout (AbortController). While waiting: spinner + t("intake.loading"); after 6 s a muted t("intake.loadingSlow").
...
7. After every response apply the prefill again (the client wins): if prefill.task_type, set draft.task_type to it; copy every non-empty prefill.organisation field (name, phone, category, resource_id) over draft.organisation; if organisation name and phone are now known, remove missing items whose field starts with "organisation"; always remove missing items whose field matches /child.*name/i (we never ask for a child's name). The server does all of this (since 13:09; child names since 15:05, DEF-050); this is a one-line guard only.
8. Keep { draft, prefill, round } in an IntakeProvider (React context) only (no sessionStorage: Knowledge HARD RULES). Never send the free text anywhere else and never write it to Supabase.
9. Fallback when the request fails, times out or returns 502: brick-tint panel t("intake.parseErrorTitle") with t("intake.parseRetry") and t("intake.parseManual"). The second builds a full draft locally: { task_type: prefill?.task_type ?? "other_call", user_language: uiLang, person: { name: "" }, organisation: prefill?.organisation ?? { name: "", phone: "", category: "", resource_id: null }, resource_id: prefill?.organisation?.resource_id ?? null, goal_user: text.slice(0, 200), goal_de: "", time_windows: [], constraints: {}, allowed_facts: [], reason_category: null, is_new_patient: null, has_referral: null, insurance_type: null, insurance_name: null, opening_de: "", opening_user: "", sensitive_removed: false, refuse_reason: null, ready: false, missing: [], essentials_missing: course_enquiry -> ["organisation.phone" (only if empty), "person.name", "fact:child_age"]; kita_enquiry -> ["organisation.phone" (only if empty), "person.name", "fact:child_age|child_birth", "fact:start_month"]; doctor_appointment -> ["organisation.phone", "person.name", "reason_category", "is_new_patient", "insurance_type", "time_windows"]; others -> ["organisation.phone", "person.name", "goal_user"] }. P8 asks them with its essentials legends.
10. refuse_reason: "emergency" -> a role="alert" panel t("intake.refuseEmergency") with tel: links (numbers in <bdi dir="ltr">), focus its heading, no Continue; "not_by_phone" -> t("intake.refuseNotByPhone") + link to /library; "unsafe" -> t("intake.refuseUnsafe").
11. If sensitive_removed: a muted line with a shield icon t("ask.sensitiveRemoved").
...
**Check it worked:** from a course page, "Ask for me" opens `/ask` with "We will call: <name>". Type the Russian demo sentence → DevTools Network shows exactly **one** POST to n8n (plus its CORS preflight) whose body has `"ui_lang":"ru"` and a `prefill` object, and no Supabase write. The goal sentence appears in Russian and the place stays the provider even if the server draft comes back without it. With `VITE_INTAKE_URL` pointing to a wrong URL, "Fill in the details myself" works.
...
2. Name in Latin letters: if draft.person.name contains letters that are not Latin script (use /\p{L}/u and /\p{Script=Latin}/u), show a required text field t("ask.latinName") with help t("ask.latinNameHelp"). Prefill it with a simple Cyrillic-to-Latin letter map (for example Мария Иванова -> Mariya Ivanova); leave it empty for other scripts. Use this value as patient_name, and replace the original name inside opening_de and inside every allowed_facts value before showing the card. latinName = this field when it is shown, else draft.person.name (the live intake already transliterates, e.g. "Maria Ivanova", so the field is a guard). If the name is empty (fallback draft), show the same field as required with label t("intake.personNameLabel").
3. Approval card t("approval.title") (paper-deep, radius 10, 1px border, no shadow), in this order:
   a) opening_de in <De> (Fraunces), and under it opening_user in <UserText lang={draft.user_language}> (muted). If user_language is "de", show only the German line. If opening_de is empty (fallback draft), skip a).
   b) t("approval.goalLabel"): goal_user + place name + phone (<bdi dir="ltr">).
   c) For course_enquiry and kita_enquiry: t("ask.willAsk") + t(`ask.willAsk_${task_type}`,{month: the start_month fact value}).
   d) t("ask.yourTimes"): each window with formatRange; t("approval.readBack"); then t(`ask.mayAgree_${task_type}`) for course/kita; if there are no windows: t("ask.noWindows").
   e) t("approval.factsLabel"): allowed_facts as checkboxes in 44px rows, all ticked; unticked facts are removed before insert. For course/kita add a muted line t("ask.noChildName").
   f) t("approval.neverLabel"), 3 muted lines with icons (shield-check, lock, calendar-x): t("approval.never1") / t("ask.neverHealthChild") (doctor tasks: t("approval.never2")) / t("ask.neverTimes").
   g) No email field (Knowledge HARD RULES): user_email is always null.
   ...
   i) Primary t("ask.callNow"), disabled until consent is ticked, latinName and organisation.name are not empty, organisation.phone matches ^(\+49|0)\d{6,14}$ after stripping, and (doctor_appointment only) reason_category is set; otherwise show the missing field on the card. Text button t("approval.changeSomething") returns to the questions with all answers kept (no separate edit form).
4. Insert on "Call now" (spinner t("intake.insertStarting")):
   const id = crypto.randomUUID();
   const { error } = await supabase.from("call_requests").insert({ id, task_type: d.task_type, user_language: d.user_language, patient_name: latinName, user_email: null, practice_name: d.organisation.name, practice_phone: d.organisation.phone, organisation_category: d.organisation.category || null, resource_id: prefill?.organisation?.resource_id ?? d.organisation.resource_id ?? null, goal_user: (d.goal_user ?? "").slice(0, 200) || null, reason_category: d.reason_category ?? null, insurance_type: d.insurance_type ?? null, insurance_name: d.insurance_name ?? null, has_referral: d.has_referral ?? false, is_new_patient: d.is_new_patient ?? true, time_windows: d.time_windows, constraints: d.constraints ?? {}, allowed_facts: checkedFacts, consent_ai_call: true });
   Never send status, goal_de, call_brief_de or created_at; send arrays and objects as real JSON, never JSON.stringify. On success clear the draft (context + sessionStorage) and navigate({ to: "/r/$id", params: { id } }). On error a brick panel t("intake.insertError") with t("errors.codeLabel",{code: error.code}) in small muted text.
...
```text
Build the live state of src/routes/r.$id.tsx and set ssr: false on that route (Realtime, timers, window.open and matchMedia run only in the browser); keep its head() with the title "Live call · Dresden mit Kind" and add { name: "robots", content: "noindex" }. Read-only. Do NOT create or change tables. All strings via t(); dates via src/lib/format.ts.
Data: call_requests by id with select("id,status,task_type,user_language,practice_name,practice_phone,resource_id,goal_user,time_windows,allowed_facts,created_at") (never user_email or patient_name) .maybeSingle(); all calls rows for the request (order created_at desc) and pick ONE: the newest call that is still running (ended_at is null) or has an outcome; skip calls that ended without an outcome (aborted tests on the relay page); if none qualifies, the newest call. transcript_lines for that call ordered by id; events ordered by id. Realtime, one channel per table: call_requests UPDATE (id=eq.<id>), calls INSERT and UPDATE (request_id=eq.<id>), events INSERT (request_id=eq.<id>), transcript_lines INSERT and UPDATE (call_id=eq.<callId>; text_user arrives later by UPDATE). When the picked call changes (a calls INSERT after "Start the call"), remove the transcript_lines channel, subscribe again with the new call id and refetch its lines. Refetch once on SUBSCRIBED, remove all channels on unmount. Every visible state comes from database rows only.
1. Start the call: while no calls row exists and status is "submitted" or "briefed", show a paper-deep panel: heading t("call.readyTitle",{place: practice_name}); text t("call.readyText"); primary button t("call.start") that runs window.open(`${RELAY_URL}/?request=${id}`, "_blank", "noopener"). While status is "submitted" add a muted line t("call.briefing"). After the click the button label becomes t("call.started") (it stays clickable, focus stays on it). Under it a <details> t("call.trouble") with a link t("call.local") -> `${RELAY_URL_LOCAL}/?request=${id}` (new tab). If RELAY_URL is empty, the primary button opens ${RELAY_URL_LOCAL}/?request=${id} and the <details> is hidden. Use only these two base URLs, never one from the query string. Hide the panel as soon as a calls row exists.
...
3. Call card: place name (display 26), phone, timer mm:ss from calls.started_at in <span role="timer" dir="ltr" class="tabular-nums"> (not a live region), and ONE role="status" phase line: "Preparing…" (no call) / "Dialling…" (call, no lines) / "Connected" (lines) / t("call.phaseEnded",{duration}) (ended_at). Who is speaking: "Assistant" (bot icon) and the other side from t(`otherSide.${task_type}`) (course_enquiry "Course", kita_enquiry "Kita"); the side of the latest line shows 3 small pine bars for 3 s (aria-hidden); under prefers-reduced-motion show the word "speaking" instead. Trust line with shield-check: if any agent text_de contains "KI" -> "The assistant said it is an AI at the start of the call." else "The assistant will say it is an AI in its first sentence."
...
```text
On /r/:id add the result state. When status is booked, completed, needs_user, rejected or failed (or the call picked in P9 has an outcome), show the result card at the top of the main column and collapse the call card to one line t("result.endedSummary",{duration}) · t("result.showFullCall"). Use summary_user if present, else summary_en (in <UserText lang={user_language}>). Read-only.
1. booked: the only card with the soft shadow (0 1px 2px rgb(28 36 32/.06), 0 4px 12px rgb(28 36 32/.06)). Heading by calls.result?.booking_kind: "trial_lesson" -> t("result.trialBooked"), "kita_visit" -> t("result.kitaVisitBooked"), else t("result.bookedTitle"). Big line booked_slot via formatDateTime (display 33). Place + phone. "Bring with you": each bring_items entry with a check icon, the German word in <De> and the meaning from bring_items_user?.find((b) => b.de === word)?.meaning_user (bring_items_user is an array [{de, meaning_user}] written by n8n 06) ?? t(`germanTerms.${word}`) (no meaning line if that is "") (Hausschuhe "indoor shoes", Sportsachen "sports clothes", Versichertenkarte "health insurance card", Impfpass "vaccination record", Überweisung "referral letter"). Then the details list (3). Primary "Add to my calendar"; text link t("result.backTo",{place}) -> /p/<resource_id> if resource_id, else t("result.startAnother") -> /ask.
...
5. rejected / rejected_no_new_patients: brick-tint; heading t("result.noPlace") (doctor_appointment: t("result.rejectedNoNewPatients")); primary "Find another place" -> /courses for course_enquiry, /services?group=kita_school for kita_enquiry, else /services.
...
8. Calendar file built in the browser (dresden-mit-kind-<task_type>.ics): VCALENDAR/VEVENT with UID <id>@dresden-mit-kind, DTSTAMP now (UTC), DTSTART booked_slot (UTC), DTEND +60 min for trial_lesson and kita_visit, else +30 min, SUMMARY t("result.calendarSummary",{task: heading, place: practice_name}), LOCATION practice_name, DESCRIPTION t("result.calendarDescription",{items, phone}); build it with a new src/lib/ics.ts (UTC DTSTART/DTEND or VALUE=DATE, CRLF, escaping; P4 reuses it) CRLF line endings; escape , ; \ and newlines.
When the status becomes final, move focus once to the result heading (no aria-live on the card).
```

**Check it worked:** `/r/0000…0001` still shows the booked card at 08:15 Berlin time (reset on 26 Sep: status `booked`, only the mock call). In the SQL editor set a test call to `outcome='booked'`, `result='{"result_type":"booking","booking_kind":"trial_lesson","date":"2026-10-01","time":"16:30","free_spot":true,"language_of_instruction":"Russisch und Deutsch"}'` → the card reads "Trial lesson booked", Thursday 1 Oct, 16:30, with "Free spot: Yes" and the German text marked "(in German, as they said it)". A `completed` call with `result_type: kita_availability` shows "What the Kita said".
...
Home:
1. Hero, asymmetric split (start 7/12, end 5/12; stacked on mobile, text first). H1 t("home.title") in display 52px (36px mobile), Fraunces "SOFT" 50. Subline t("home.subtitle") (21px, max 34rem). End column: the Ask-for-me box (paper-deep, radius 10, 1px border): label t("home.askLabel"), textarea (3 rows, dir="auto", maxLength 600), helper t("home.askExample"), primary button t("common.continue") -> navigate({ to: "/ask", state: { text } }) (HistoryState from P7). Never put the text in the URL.
2. Age band, full-bleed on sand: heading t("home.ageQuestion") and 5 large links (min 48px high, radius 6, paper background, 1px pine outline because the grey input outline is too weak on sand) labelled t(`taxonomy.age_${id}`) for 0-1, 1-3, 3-6, 6-10, 10+ (Arabic uses a hyphen; an en dash between digits is shown reversed in RTL) -> <Link to="/courses" search={{ age: id }}>, aria-label t("home.ageLinkLabel",{band: t(`taxonomy.age_${id}`)}).
3. t("home.pillarsTitle"): a two-column list (one column on mobile) of the 5 pillars with lucide icons (Music, CalendarDays, Languages, Stethoscope, BookOpen), the pillar name as a link and one line (pillarCourses, pillarEvents, pillarCommunities, pillarServices, pillarLibrary). Show t("home.placesCount",{count}) only for Courses (resources in PAGE_CATEGORIES.courses) and Health & services (PAGE_CATEGORIES.services), and only if the head count query returns > 0: supabase.from("resources").select("id", { count: "exact", head: true }).in("category", PAGE_CATEGORIES.courses); for Health & services add .or("category.neq.community,audience.neq.family") so the count matches /services. Run both counts and the step-4 events query in the route loader (SSR). No counts for the other pillars. No cards.
...
- validateSearch { kind?, resource? }; if resource is a UUID: load the name (resources, maybeSingle) and show t("suggest.about",{name});
- name: required, 2–160 characters, label t("suggest.name");
- url: optional, label t("suggest.url"), must start with https:// (matches t("suggest.urlError")), max 300;
- note: optional textarea, max 500, counter, label t("suggest.note");
- above the button a muted line t("suggest.privacy");
- honeypot: an input name="company_fax" positioned off-screen, tabIndex -1, autoComplete "off", aria-hidden="true", label t("suggest.honeypot"). If it has a value, or the form is sent less than 3 seconds after the form mounted (take the time in useEffect, not during render), show the success message but insert nothing.
Insert: const { error } = await supabase.from("suggestions").insert({ kind, name, url: url || null, note: note || null, ui_lang: uiLang, resource_id: resourceId || null }); Do NOT add .select() (the browser is not allowed to read this table). Success: replace the form with t("suggest.thanks") and links t("suggest.backHome") -> / and t("suggest.browseCourses") -> /courses. Error: role="alert" t("suggest.error"), keep the values. Field errors inline with icon + text (aria-invalid, aria-describedby).
/demo stays a redirect to /r/00000000-0000-0000-0000-000000000001.
head() for /: title "Dresden mit Kind · courses, events and help for families in Dresden", description = the English home.subtitle, og:title, og:description; /suggest: "Suggest a place · Dresden mit Kind".
```
...
13. SSR audit: move any window, document, localStorage, sessionStorage, navigator or matchMedia use during render into useEffect or event handlers; add suppressHydrationWarning only on <time> elements; no hydration warnings in the console on /, /courses, /p/<id> in en and ar.
14. Every route has a head() title "<page> · Dresden mit Kind" and a one-sentence description (the favicon comes from P14).
...
```text
On src/routes/r.$id.tsx add a secondary text button "Take the call here (demo)" next to "Start the call", shown only when the search param demo is "1" (validateSearch) and RELAY_WS_URL from src/config.ts is not empty (P1b derives it from RELAY_URL: https -> wss). On click: ask for microphone permission, open new WebSocket(`${RELAY_WS_URL}/call?request=${id}`), and use the helper src/lib/relayCall.ts (already in the repo, copied from the relay page) to stream 16 kHz mono microphone audio and play the agent audio. Show a "Hang up" button; after the socket opens move focus to it, after it closes move focus back. Errors: NotAllowedError -> "The microphone is blocked. Click the lock icon in the address bar, allow the microphone, then try again."; NotFoundError -> "No microphone found. Plug one in or use headphones with a mic."; socket closed before open -> "Could not reach the call service." Never store audio.
```

Before P13: Claude Code copies the call logic of `services/voice-relay/public/index.html` + `mic-worklet.js` into the Lovable repo (`src/lib/relayCall.ts`, `public/mic-worklet.js`), The current relay `server.js` already accepts `https://*.lovable.app` and `https://*.lovableproject.com` (`ALLOWED_ORIGIN_SUFFIXES` default); once our URL is fixed, put the exact origin in `ALLOWED_ORIGINS` and set `ALLOWED_ORIGIN_SUFFIXES=` empty, so other Lovable apps cannot use our Deepgram quota (`docs/deploy-relay.md` documents this since 26 Sep). **Check it worked:** with `?demo=1`, the call runs in the same tab and the transcript appears as with the relay tab.

#### P14: Launch pass: legal pages and SEO head tags (sent 26 Sep, built)

Before sending (done via GitHub, zero credits): `src/content/impressum.md` and `src/content/datenschutz.md` (German + English; operator details are **[placeholders]** the team must fill in), `public/robots.txt`, `public/sitemap.xml` (107 URLs, no demo listing), `public/og.png`, `public/favicon.svg`; keys `common.footerImpressum`, `common.footerPrivacy`, `legal.*` in 6 languages (footer links exist since P1b).

```text
Launch pass: legal pages and SEO head tags. No new features, no schema changes, no Lovable Cloud, no auth, no analytics, no cookies.
...
2. src/routes/impressum.tsx and src/routes/datenschutz.tsx exist as placeholders: make each render the Markdown file that is already in the repo (src/content/impressum.md, src/content/datenschutz.md; import with ?raw) with react-markdown (add it; no raw HTML, no rehype-raw). Wrap the rendered text in lang="de". Above it, when uiLang is not "de", a muted line t("legal.germanBinding"). Do not write, shorten, translate or "improve" the legal text, and never invent names, addresses or emails (the [placeholders] stay as they are). head(): title "Impressum · Dresden mit Kind" / "Datenschutz · Dresden mit Kind".
3. Create src/lib/seo.ts: seo({ title, description, path, noindex }) returns { meta, links } for head(): title; description (max 160 characters, cut at a word boundary); og:site_name "Dresden mit Kind"; og:type "website"; og:title; og:description; og:url and <link rel="canonical"> = SITE_URL + path (no query, no hash); og:image SITE_URL + "/og.png" with og:image:width 1200 and og:image:height 630; twitter:card "summary_large_image"; if noindex: { name: "robots", content: "noindex" }. Use it in every route's head() (keep the existing titles and descriptions; the English texts come from src/i18n/strings/en.json because the server does not know the visitor's language). noindex on /ask, /r/$id, /demo, /suggest, the 404 and the demo listing (subcategory "demo", already done in p.$id.tsx). Never put a canonical in the root route.
4. Root head(): link rel="icon" type="image/svg+xml" href="/favicon.svg" (keep /favicon.ico as fallback), theme-color "#F7F4EC", default og:site_name, og:image and twitter:card.
5. Do not create, change or delete anything in public/ (robots.txt, sitemap.xml, og.png, favicon.svg come from GitHub) and add no sitemap or robots server routes. No hreflang, no language prefixes in URLs.
Report the files you changed.
```

After P14: Lovable's security scan reports 1 critical finding ("Exposed personal & sensitive data" = the demo-mode public read on `call_requests`). Never click Lovable's SQL "fix": the fix is owner-scoped RLS + auth in our repo (Monday). Settings used at publish: URL `dresden-mit-kind`, badge hidden, visitor analytics **off**, auto-fix off.

#### P15: Dictation button (sent 26 Sep, built)

```text
Add dictation (voice input) to the Tell box on /ask and to the Ask box on the home page. It uses our own voice relay's speech-to-text (Deepgram behind the relay; nothing is stored). No new dependencies, no other speech service (no Web Speech API: it sends audio to Google), no table changes.
1. Create src/components/ask/DictateButton.tsx with props { lang, onFinalText }. It renders an outline button (44px, radius 6): lucide Mic + t("intake.speakInstead"); while listening: lucide Square + t("intake.speakStop"), aria-pressed="true" and a small pine dot (aria-hidden). Everything that touches navigator, AudioContext or WebSocket runs only in the click handler or effects (SSR-safe).
2. Start (click): navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, channelCount: 1 } }); const ctx = new AudioContext(); await ctx.audioWorklet.addModule(<Blob URL of the worklet in step 6>); const node = new AudioWorkletNode(ctx, "mic-downsampler"); ctx.createMediaStreamSource(stream).connect(node). Open const ws = new WebSocket(`${RELAY_WS_URL || "ws://127.0.0.1:8787"}/listen?lang=${code}`) (RELAY_WS_URL from src/config.ts). code = lang if it is one of en ar tr uk ru fa hi es fr pl vi zh de, else "en". node.port.onmessage = (e) => { if (ws.readyState === 1) ws.send(e.data) } (binary Int16 PCM, 16 kHz).
3. Messages from the relay are JSON: {type:"ready"} | {type:"transcript", text, is_final} | {type:"error", message} | {type:"closed"}. On is_final: call onFinalText(text) (the parent appends it to the textarea value with a space, respecting maxLength). Interim text: show it muted under the textarea (aria-hidden), never in the value.
4. Stop (click again, or automatically after 120 s, or when the component unmounts): ws.send(JSON.stringify({ type: "stop" })) if open, stop all tracks, close the AudioContext. The relay sends the last final result and then closes.
5. Errors, shown inline under the textarea in a role="status" line (typed text stays): NotAllowedError -> t("errors.micDenied"); NotFoundError -> t("call.micNotFound"); socket closed before "ready", or {type:"error"} -> t("intake.micError"). The textarea always stays editable.
6. Worklet source for the Blob (exactly this, it matches our relay):
class MicDownsampler extends AudioWorkletProcessor {
  constructor() { super(); this.ratio = sampleRate / 16000; this.buffer = []; this.pos = 0; this.acc = 0; this.n = 0; }
  process(inputs) {
    const ch = inputs[0] && inputs[0][0];
    if (!ch) return true;
    for (let i = 0; i < ch.length; i++) {
      this.pos += 1; this.acc += ch[i]; this.n += 1;
      if (this.pos >= this.ratio) { this.buffer.push(this.acc / this.n); this.pos -= this.ratio; this.acc = 0; this.n = 0; }
    }
    if (this.buffer.length >= 1600) {
      const out = new Int16Array(this.buffer.length);
      for (let i = 0; i < this.buffer.length; i++) { const s = Math.max(-1, Math.min(1, this.buffer[i])); out[i] = s < 0 ? s * 0x8000 : s * 0x7fff; }
      this.port.postMessage(out.buffer, [out.buffer]); this.buffer = [];
    }
    return true;
  }
}
registerProcessor("mic-downsampler", MicDownsampler);
7. Place DictateButton in /ask step 4 next to the character counter under the Tell textarea, and in the home Ask box under its textarea (lang = uiLang). The dictated text is treated exactly like typed text: React state only, never stored, never in the URL.
Report the files you changed.
```

**Note:** dictation and the call use the relay. On the presenter laptop the local relay (`127.0.0.1:8787`) works; other devices need the public relay host (`docs/deploy-relay.md`).

#### P16: Language switch without cookies (sent 26 Sep, built)

```text
Fix: the language switch must also work when the browser blocks the cookie (for example inside the Lovable editor preview, which is a cross-site iframe where Chrome drops the dmk_ui_lang cookie, and in strict-privacy browsers). In src/i18n/index.tsx keep client state { ui, strings } in LanguageProvider, initialised from the root loader data. setUiLang(code), in the click handler only: 1) write the cookie as today (it still decides the server-rendered language next time); 2) load the strings in the browser at once: code === "en" ? en : (await import(`./strings/${code}.json`)).default; 3) set the state and document.documentElement.lang / .dir (rtl only for ar); 4) then router.invalidate(). When fresh loader data arrives, use it only if its ui equals the chosen language (so a blocked cookie never switches the page back to English). Everything that reads t() and ui must use this state. Keep SSR as it is (no localStorage, nothing read during render). No other changes.
```

### G.3 Credit-saving tips

1. **Strings, translations, RTL renames, Supabase types and static files** (robots, sitemap, og image, favicon, legal Markdown) **in the Lovable repo via GitHub** (Claude Code), not in Lovable. Zero credits. Keep `main` working, never force-push.
2. **Test with SQL, not prompts** (G.4): 0 Lovable credits, but 2 n8n executions (05 + 06) per run.
3. **Questions about what Lovable built: ask Claude Code against the local clone** (zero credits); Lovable Chat mode also uses credits. Agent mode only to build. One prompt = one step; two strikes → fix in the repo.
...
6. **Budget:** P1 cost 2.2 credits for a 1.9k-character prompt; P1b–P12 are 1.5k–7.5k characters with far more logic, so expect ≈ 45–70 credits with one fix each, + ≈ 3 for P14. Keep ≥ 30 for Sunday morning; skip P13 unless everything else is done.

### G.4 Test script: simulate a course call (Supabase SQL editor; 0 Lovable credits, 2 n8n executions)
...
```sql
-- after P8: your new request id = <ID> (replace every <ID>). Run one statement at a time and watch /r/<ID>.
-- 0 Lovable credits; n8n 05 translates the practice line and n8n 06 rewrites summary_user + bring_items_user when the outcome is set (2 n8n executions).
-- provider 'mock': the DEF-047 trigger (migration 20260926220000) never sets "Checked by phone", so no clean-up is needed.
update call_requests set status='briefed', goal_de='Nach einem freien Platz oder einer Probestunde für ein vierjähriges Kind fragen.' where id='<ID>' and status='submitted';
insert into calls (request_id, provider, started_at, disclosure_variant) values ('<ID>','mock',now(),'warm_default');
update call_requests set status='calling' where id='<ID>';
insert into transcript_lines (call_id, speaker, text_de) select id, 'practice', 'Olgas Musikstudio, guten Tag?' from calls where request_id='<ID>' order by created_at desc limit 1;
insert into transcript_lines (call_id, speaker, text_de) select id, 'agent', 'Guten Tag! Hier ist die KI-Assistentin von Maria Ivanova. Ich rufe für Maria Ivanova an und wollte fragen, ob es in Ihrem Kurs für Vierjährige noch einen Platz oder eine Probestunde gibt.' from calls where request_id='<ID>' order by created_at desc limit 1;
update transcript_lines set text_user='Здравствуйте! Это ИИ-ассистент Марии Ивановой. Я звоню от её имени и хочу спросить, есть ли в вашем курсе для четырёхлетних детей свободное место или пробное занятие.' where speaker='agent' and text_user is null and call_id=(select id from calls where request_id='<ID>' order by created_at desc limit 1);
update calls set ended_at=now(), outcome='booked', booked_slot='2026-10-01 16:30+02', bring_items=array['Hausschuhe'],
  ...
  summary_user='Пробное занятие записано: четверг, 1 октября, 16:30. Возьмите сменную обувь. Курс на русском и немецком языках.'
  where id=(select id from calls where request_id='<ID>' order by created_at desc limit 1);
update call_requests set status='booked' where id='<ID>';
-- The "Checked by phone" badge only comes from real relay calls (provider deepgram-browser, parent name without "test"/"Roleplay"/"(E2E").
-- After such a test on a real listing: update resources set last_checked_at=null, last_check_outcome=null where id='<RESOURCE_ID>';
```
...
| 0:15–0:35 | **Discover** | Tap **3–6** → `/courses?age=3-6`; add Activity "Музыка", Language "Русский", District "Neustadt" (chips appear, count updates) → open the provider: description in Russian, age ruler 3–6, languages "Русский · Deutsch", price "Пробное занятие возможно", Ask-for-me box | P2, P3, demo listing Olgas Musikstudio (exists; age 3–6 + Музыка + Русский = "Мест: 2", + Neustadt = "1 место", checked live) |
...
| 2:15–2:35 | **Result** | Result card in Russian: "Пробное занятие записано", Thursday 1 Oct 16:30, bring **Hausschuhe** (сменная обувь), "Free spot: Да", **Add to my calendar**; back on the provider page the badge "Проверено по телефону …" (the demo request uses "Maria Ivanova" and the relay's provider `deepgram-browser`, which the DEF-047 trigger does not skip; never use a name containing "test") | P10, P3 |
...
**Demo listing rule:** "Olgas Musikstudio" is demo data (Pitch Kit v2). It is in `resources` since RUN-025 (id `ae0ae23b-0dd7-4f82-adf8-6fd78aa3456d`, `subcategory = 'demo'`, fictional phone `+49 351 0000000`, Musterstraße 1, Neustadt, music, 3–6, `ru` + `de`, `trial_available`, `i18n` in 5 languages, no website, no lat/lng), so the UI shows "Demo listing, not a real business" everywhere; never role-play a real Dresden provider on stage and never present invented answers as theirs.
...
3. Live voice fails → run G.4 on the same request (the live screen fills from SQL), then the backup video. G.4 inserts `provider = 'mock'`, which the DEF-047 trigger skips: no "Checked by phone" badge in this path (skip that beat or change G.4's provider).
...
|---|---|---|
| Demo listing "Olgas Musikstudio" | Done (RUN-025) | In `resources` since RUN-025 (see the Demo listing rule above); `/courses?age=3-6&act=music&lang=ru` shows 2 places (Olgas + Musikschule Adagio), adding `district=Neustadt` leaves only Olgas; recheck Sunday 10:00 |
| Russian/German UI strings don't exist yet | Done 26 Sep | `docs/i18n/ui-strings.json` has all 6 languages (789 keys each since RUN-025: `subcategories.german_for_children`; the Lovable repo copy already has it); split into the Lovable repo (G.0 step 2); native-speaker check for ru, uk, ar still open |
...
| The intake asks for the child's name (`missing` field `child_first_name`, seen 13:20) | Fixed ([[DEF-050 Intake asks for the child's first name]], RUN-025) | n8n 04 "Validate + Clean Draft" drops child-name facts, notes and questions (`/child.*name/i`); P7/P8 keep the client drop as a guard |
| `/demo` request `…0001` was stuck (`calling` + an aborted call, 26 Sep morning) | Reset (checked 26 Sep evening: `status = booked`, only the seeded mock call `…00c1`; the DB holds just the 2 seeded requests) | The relay page and `server.js:358` still default to this ID and the relay re-opens finished requests (no status check), so any relay test without an ID writes onto it again: re-check at 12:30 with `select status from call_requests where id = '00000000-0000-0000-0000-000000000001';`; always test with your own request ID |
| Test calls against real listings set "Checked by phone" ([[DEF-047 Role-play calls mark real providers as checked by phone]]) | Fixed for marked tests (migration `20260926220000`) | The trigger skips `provider` test/verify/mock and names containing "(E2E", "roleplay" or "test". Browser role-plays with an ordinary name (`deepgram-browser`) still set it, so the badge shows only on the demo listing (C3); the presenter uses "Maria Ivanova" (no "test") so the demo badge appears |
...
| Phones (26 Sep 15:05, RUN-025): 71 of the 84 service rows (doctors 37/38, pharmacies 16/16, banks 7/14: the others publish only a central hotline) and 77 of 92 family rows | Known | "No phone number listed" + the intake asks for the number; never invent |
| 46 of 92 family rows have no age range (on `/courses`: 15 of 51), so the age filter moves many rows aside | Known | They sit in the closed "{count} places don't list ages" group; the Discover beat uses the demo listing (ages 3–6) |
...
| Assistant lines get no live subtitle (migration `20260926210000`: only the other side is translated live); n8n 06 fills them in one batch ~15 s after the outcome (live since RUN-024 and in the export) | Known | P9 shows no placeholder under agent rows, shows `text_user` when it arrives and never announces late subtitles; the approval card already showed the opening in the user's language |
| Credits run out | Low–Medium | ≥ 30 reserve; text and RTL fixes in the repo; skip P13 |
| Demo-mode RLS: anyone with the public anon key can list all requests (name, phone, goal, facts), not only by link | High on a public URL | No email field (F.6); demo notice "Please use test data only" in the footer, on `/ask` and `/r/:id`; `noindex` on `/r/*`; owner-scoped policies are a Monday-morning item |
| Public URL spends our credits: every intake POST = 1–2 Claude calls (n8n 04; the Origin check stops browsers on other sites, not scripts), every `call_requests` insert = 1 Claude brief (n8n 01), every relay call = Deepgram minutes + Claude; anon can insert without limit | High | Check n8n credits Sunday 10:00. After the demo take the hosted relay down or set its `ALLOWED_ORIGINS` to localhost only. Kill switch: delete the Vault secrets `n8n_new_request_url` / `n8n_translate_line_url` (the triggers become no-ops) and deactivate n8n 04 |
| Fake "Checked by phone" on real listings (anyone can create a request for a real listing, press Start the call and play the place) | High on a public URL | Badge only for `subcategory = 'demo'` (C3); if it happens, reset `last_checked_at` / `last_check_outcome` |
| Custom domain breaks the intake | High if a custom domain is attached | n8n 04 accepts only `*.lovable.app`, `*.lovableproject.com`, `localhost:8787`, `127.0.0.1:8787` ("Check Origin + Size", lines 5–10): every request gets 403 `origin_not_allowed`. Add the domain there (and to the relay's `ALLOWED_ORIGINS` for P13) before switching the URL ([[Deploy, Domain and SEO]]) |
| n8n credits run out before or during the demo | Medium | n8n 01 then sets `failed` before any call: the Start panel stays for `failed` (C7) and the relay uses its template brief; live subtitles (05) and the summary (06) would be missing, the German lines still show |
| SSR language flash or hydration errors (TanStack Start renders on the server) | Medium | Cookie-based `uiLang` (E row 2); test a hard reload of `/courses` in العربية and in Русский |
| Public German site without Impressum / Datenschutz, fonts loaded from Google | High for the public launch | Static `/impressum` (placeholders the team fills in) + `/datenschutz` (B); self-hosted fonts (D3) |
| "No audio" claim vs Deepgram model-improvement retention | Fixed (26 Sep evening) | The relay sends `mip_opt_out: true` in the Voice Agent Settings message and on `/listen`; a no-DB test run got `SettingsApplied` |
| No production error monitoring | Known | `reportLovableError` works only in the Lovable editor preview; n8n 04 saves no error executions. During the rehearsal watch the Supabase `events` table and the n8n executions of 01/05/06 |
